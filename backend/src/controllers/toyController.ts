...
import { getToySchema, toyCreateSchema, toyListSchema, toyUpdateSchema } from "../schemas/toyValidationSchemas";
...
export const ToyController = {
  ...
  async getToyList(req: FastifyRequest<{ Body: toyListBody }>, reply: FastifyReply) {
    try {
      ...
      const result = await ToyService.searchToys(userId, data.page, data.pageSize, data.filter);

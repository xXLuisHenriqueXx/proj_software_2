...
export const ButtonNew = styled(ButtonSend)`
  background-color: transparent;
  border: 1px solid ${({ theme }: ITheme) => theme.colors.highlight};
`;

export const TextNew = styled(TextSend)`
  color: ${({ theme }: ITheme) => theme.colors.highlight};
`;

export const ContainerRemainingTime = styled.View`
  flex-direction: column;
  align-items: center;
  justify-content: center;
  width: 100%;
...
export const TextRemainingTime = styled.Text`
  font-size: 16px;
  font-family: ${({ theme }: ITheme) => theme.fonts.redHatDisplayRegular};
  color: ${({ theme }: ITheme) => theme.colors.highlight};
  text-align: center;

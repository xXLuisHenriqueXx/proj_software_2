...
type Props = NativeStackScreenProps<AppStackParamList, "UpdateProduct">;
    ...
    try {
      ...
      if (response) {
        const fields = {
          ...
          canLend: response.data.canLend,
          usageTime: response.data.usageTime,
          type: response.data.type,
          ageGroup: response.data.ageGroup,
          pictures: response.data.pictures.map((item: any) => item.picture),
  ...
  return (
    ...
    >
      <Header.Root padding={16}>
        <Header.Content title="Atualizar anúncio" />
